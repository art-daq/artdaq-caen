    ...
    : CommandableFragmentGenerator(ps), fCAEN(ps), fAcqMode(CAEN_DGTZ_SW_CONTROLLED)
{
	...
	TLOG(TDEBUG) << " Calling CAEN_DGTZ_OpenDigitizer(" << CAEN_DGTZ_OpticalLink << ", " << fCAEN.link << ", "
	             << fBoardChainNumber << ", " << 0 << " , " << &fHandle << ")";  // AA: debug
...
void artdaqcaen::CAENV1730Readout::configureInterrupts()
{
	...
	TLOG(TLVL_INFO) << "Configuring Interrupts state=" << uint32_t{stateOut}
	                << ", interruptLevel=" << uint32_t{interruptLevelOut} << ", statusId=" << uint32_t{statusIdOut}
	                << ", eventNumber=" << uint32_t{eventNumberOut} << ", mode=" << int32_t{modeOut};
	...
	retcode =
	    CAEN_DGTZ_GetInterruptConfig(fHandle, &stateOut, &interruptLevelOut, &statusIdOut, &eventNumberOut, &modeOut);
	...
	{
		TLOG_WARNING("CAENV1730Readout") << "Interrupt StatusID was not setup properly, eventNumber write/read="
	...
	uint32_t data =
	    (uint32_t)(0x9);  // RORA,irq link enabled,vme baseaddress relocation disabled,VME Bus error enabled,level 1
	...
	retcode = artdaqcaen::CAENV1730Readout::WriteRegisterBitmask(fHandle, addr, data, bitmask);
	artdaqcaen::CAENDecoder::checkError(retcode, "SetIOControl", fBoardID);
	retcode = CAEN_DGTZ_ReadRegister(fHandle, addr, &value);
...
void artdaqcaen::CAENV1730Readout::loadConfiguration(fhicl::ParameterSet const& ps)
{
...
void artdaqcaen::CAENV1730Readout::Configure()
{
	...
	artdaqcaen::CAENDecoder::checkError(retcode, "SetAcquisitionMode", fBoardID);
	...
	artdaqcaen::CAENDecoder::checkError(retcode, "SWStopAcquisition", fBoardID);
...
void artdaqcaen::CAENV1730Readout::RunADCCalibration()
{
	TLOG_ARB(TINFO, TRACE_NAME) << "Running calibration..." << TLOG_ENDL;
	auto retcode = CAEN_DGTZ_Calibrate(fHandle);
	artdaqcaen::CAENDecoder::checkError(retcode, "Calibrate", fBoardID);
}

// Animesh & Aiwu added function
void artdaqcaen::CAENV1730Readout::ConfigureOthers()
{
	...
	artdaqcaen::CAENDecoder::checkError(retcod, "Read_DPP_Algorithm_feature_0x1n80", fBoardID);
...
void artdaqcaen::CAENV1730Readout::SetLockTempCalibration(bool onOff, uint32_t ch)
{
	...
	artdaqcaen::CAENDecoder::checkError(retcod, "LockTempCalibration", fBoardID);
...
void artdaqcaen::CAENV1730Readout::Read_ADC_CalParams_V1730(int handle, int ch, uint8_t* CalParams)
{
	// int retcod = 0;
	CAEN_DGTZ_ErrorCode retcod;
	// read offset
	...
	artdaqcaen::CAENDecoder::checkError(retcod, "Read_ADC_CalParams_0x20", handle);
	retcod = ReadSPIRegister(handle, ch, 0x21, &CalParams[1]);
	TLOG(TINFO) << "Read_ADC-CalParams_ch" << ch << ": Params[1]=" << CalParams[1];
	artdaqcaen::CAENDecoder::checkError(retcod, "Read_ADC_CalParams_0x21", handle);
	retcod = ReadSPIRegister(handle, ch, 0x26, &CalParams[2]);
	TLOG(TINFO) << "Read_ADC-CalParams_ch" << ch << ": Params[2]=" << CalParams[2];
	artdaqcaen::CAENDecoder::checkError(retcod, "Read_ADC_CalParams_0x26", handle);
	retcod = ReadSPIRegister(handle, ch, 0x27, &CalParams[3]);
	TLOG(TINFO) << "Read_ADC-CalParams_ch" << ch << ": Params[3]=" << CalParams[3];
	artdaqcaen::CAENDecoder::checkError(retcod, "Read_ADC_CalParams_0x27", handle);

	// read gain
	retcod = ReadSPIRegister(handle, ch, 0x22, &CalParams[4]);
	TLOG(TINFO) << "Read_ADC-CalParams_"
	            << ": Params[4]=" << CalParams[4];
	artdaqcaen::CAENDecoder::checkError(retcod, "Read_ADC_CalParams_0x22", handle);
	retcod = ReadSPIRegister(handle, ch, 0x23, &CalParams[5]);
	TLOG(TINFO) << "Read_ADC-CalParams_"
	            << ": Params[5]=" << CalParams[5];
	artdaqcaen::CAENDecoder::checkError(retcod, "Read_ADC_CalParams_0x23", handle);
	retcod = ReadSPIRegister(handle, ch, 0x24, &CalParams[6]);
	TLOG(TINFO) << "Read_ADC-CalParams_"
	            << ": Params[6]=" << CalParams[6];
	artdaqcaen::CAENDecoder::checkError(retcod, "Read_ADC_CalParams_0x24", handle);
	retcod = ReadSPIRegister(handle, ch, 0x28, &CalParams[7]);
	TLOG(TINFO) << "Read_ADC-CalParams_"
	            << ": Params[7]=" << CalParams[7];
	artdaqcaen::CAENDecoder::checkError(retcod, "Read_ADC_CalParams_0x28", handle);
	retcod = ReadSPIRegister(handle, ch, 0x29, &CalParams[8]);
	TLOG(TINFO) << "Read_ADC-CalParams_"
	            << ": Params[8]=" << CalParams[8];
	artdaqcaen::CAENDecoder::checkError(retcod, "Read_ADC_CalParams_0x29", handle);
	retcod = ReadSPIRegister(handle, ch, 0x2A, &CalParams[9]);
	TLOG(TINFO) << "Read_ADC-CalParams_"
	            << ": Params[9]=" << CalParams[9];
	artdaqcaen::CAENDecoder::checkError(retcod, "Read_ADC_CalParams_0x2A", handle);
	// read skew
	retcod = ReadSPIRegister(handle, ch, 0x70, &CalParams[10]);
	TLOG(TINFO) << "Read_ADC-CalParams_"
	            << ": Params[10]=" << CalParams[10];
	artdaqcaen::CAENDecoder::checkError(retcod, "Read_ADC_CalParams_0x70", handle);
	// return CAEN_DGTZ_Success;
}
...
void artdaqcaen::CAENV1730Readout::Write_ADC_CalParams_V1730(int handle, int ch, uint8_t* CalParams)
{
	// int retcod = 0;
	CAEN_DGTZ_ErrorCode retcod;
	...
	artdaqcaen::CAENDecoder::checkError(retcod, "Write_ADC_CalParams_0x20", handle);
	// write offset
...
void artdaqcaen::CAENV1730Readout::ConfigureSelfTriggerMode()
{
	CAEN_DGTZ_ErrorCode retcod = CAEN_DGTZ_Success;
	// uint32_t data,readBack;

	retcod = CAEN_DGTZ_SetChannelSelfTrigger(fHandle, (CAEN_DGTZ_TriggerMode_t)fSelfTriggerMode, fSelfTriggerMask);
	artdaqcaen::CAENDecoder::checkError(retcod, "SetSelfTriggerMask", fBoardID);
}

void artdaqcaen::CAENV1730Readout::ConfigureLVDS()
{
	CAEN_DGTZ_ErrorCode retcod = CAEN_DGTZ_Success;
	uint32_t            data, readBack, ioMode;
	...
	artdaqcaen::CAENDecoder::checkError(retcod, "ReadFPOutputConfig", fBoardID);

	// Construct mode mask
	data = fModeLVDS | (fModeLVDS << 4) | (fModeLVDS << 8) | (fModeLVDS << 12);
	TLOG(TINFO) << __func__ << " ModelLVDS: 0x" << std::hex << data << std::dec;
	retcod = CAEN_DGTZ_WriteRegister(fHandle, FP_LVDS_CONTROL, data);
	artdaqcaen::CAENDecoder::checkError(retcod, "WriteLVDSOutputConfig", fBoardID);

	retcod = CAEN_DGTZ_ReadRegister(fHandle, FP_LVDS_CONTROL, &readBack);
	artdaqcaen::CAENDecoder::checkError(retcod, "ReadLVDSOutputConfig", fBoardID);
	...
	if(fModeLVDS == LVDS_TRIGGER)
	{
		...
		artdaqcaen::CAENDecoder::checkError(retcod, "ReadTRGOutputConfig", fBoardID);
		...
		artdaqcaen::CAENDecoder::checkError(retcod, "WriteTRGOutputConfig", fBoardID);

		retcod = CAEN_DGTZ_ReadRegister(fHandle, FP_TRG_OUT_CONTROL, &readBack);
		artdaqcaen::CAENDecoder::checkError(retcod, "ReadTRGOutputConfig", fBoardID);

		TLOG(TINFO) << __func__ << " TrgOutputConfig: 0x" << std::hex << data << std::dec;
	...
	TLOG(TINFO) << __func__ << " FPOutputConfig: 0x" << std::hex << ioMode << std::dec;
	retcod = CAEN_DGTZ_WriteRegister(fHandle, FP_IO_CONTROL, ioMode);
	artdaqcaen::CAENDecoder::checkError(retcod, "WriteFPOutputConfig", fBoardID);
...
void artdaqcaen::CAENV1730Readout::ConfigureRecordFormat()
{
	...
	artdaqcaen::CAENDecoder::checkError(retcode, "SetChannelEnableMask", fBoardID);
	retcode = CAEN_DGTZ_GetChannelEnableMask(fHandle, &readback);
	artdaqcaen::CAENDecoder::checkError(retcode, "GetChannelEnableMask", fBoardID);
	...
	artdaqcaen::CAENDecoder::checkError(retcode, "SetRecordLength", fBoardID);
	retcode = CAEN_DGTZ_GetRecordLength(fHandle, &readback);
	artdaqcaen::CAENDecoder::checkError(retcode, "GetRecordLength", fBoardID);
	...
	artdaqcaen::CAENDecoder::checkError(retcode, "SetPostTriggerSize", fBoardID);
	retcode = CAEN_DGTZ_GetPostTriggerSize(fHandle, &readback);
	artdaqcaen::CAENDecoder::checkError(retcode, "GetPostTriggerSize", fBoardID);
...
artdaqcaen::CAENV1730Readout::~CAENV1730Readout()
{
	TLOG_ARB(TCONFIG, TRACE_NAME) << "~CAENV1730Readout()" << TLOG_ENDL;
...
CAEN_DGTZ_ErrorCode artdaqcaen::CAENV1730Readout::WriteRegisterBitmask(int32_t  handle,
                                                                       uint32_t address,
                                                                       uint32_t data,
                                                                       uint32_t bitmask)
{
...
void artdaqcaen::CAENV1730Readout::ConfigureDataBuffer()
{
	...
	artdaqcaen::CAENDecoder::checkError(retcode, "SetMaxNumEventsBLT", fBoardID);
	...
	artdaqcaen::CAENDecoder::checkError(retcode, "MallocReadoutBuffer", fBoardID);
	...
	artdaqcaen::CAENDecoder::checkError(retcode, "FreeReadoutBuffer", fBoardID);
...
void artdaqcaen::CAENV1730Readout::ConfigureTrigger()
{
	...
	artdaqcaen::CAENDecoder::checkError(retcode, "SetSWTriggerMode", fBoardID);
	...
	artdaqcaen::CAENDecoder::checkError(retcode, "SetExtTriggerInputMode", fBoardID);
	...
	for(uint32_t ch = 0; ch < fNChannels; ++ch)
	{
		...
		artdaqcaen::CAENDecoder::checkError(retcode, "SetChannelTriggerThreshold", fBoardID);
		...
		if(ch%2==0)
		{
		  ...
		  artdaqcaen::CAENDecoder::checkError(retcode,"SetChannelTriggerPulseWidth",fBoardID);
	...
	artdaqcaen::CAENDecoder::checkError(retcode, "SetGlobalTriggerPulseWidth", fBoardID);
	...
	artdaqcaen::CAENDecoder::checkError(retcode, "SetTriggerOverlap", fBoardID);
	...
	artdaqcaen::CAENDecoder::checkError(retcode, "SetIOLevel", fBoardID);
...
void artdaqcaen::CAENV1730Readout::ConfigureReadout()
{
	...
	artdaqcaen::CAENDecoder::checkError(retcode, "SetRunSynchronizationMode", fBoardID);
	...
	artdaqcaen::CAENDecoder::checkError(retcode, "SetTestPattern", fBoardID);
	...
	addr    = DYNAMIC_RANGE;
	retcode = CAEN_DGTZ_WriteRegister(fHandle, addr, mask);
	artdaqcaen::CAENDecoder::checkError(retcode, "SetDynamicRange", fBoardID);

	addr    = ACQ_CONTROL;
	retcode = CAEN_DGTZ_WriteRegister(fHandle, addr, uint32_t{0x28});
	artdaqcaen::CAENDecoder::checkError(retcode, "SetTriggerMode", fBoardID);

	uint32_t value = 0;
	retcode        = CAEN_DGTZ_ReadRegister(fHandle, addr, &value);
	artdaqcaen::CAENDecoder::checkError(retcode, "GetTriggerMode", fBoardID);
	...
	for(uint32_t ch = 0; ch < fNChannels; ++ch)
	{
		...
		artdaqcaen::CAENDecoder::checkError(retcode, "SetChannelDCOffset", fBoardID);
...
void artdaqcaen::CAENV1730Readout::ConfigureAcquisition()
{
	...
	// CAEN_DGTZ_SetAnalogMonOutput(fHandle,(CAEN_DGTZ_AnalogMonitorOutputMode_t)(fCAEN.analogMode));
	// artdaqcaen::CAENDecoder::checkError(retcode,"SetAnalogMonOutputMode",fBoardID);
...
bool artdaqcaen::CAENV1730Readout::WaitForTrigger()
{
	...
	uint32_t acqStatus;
	retcode = CAEN_DGTZ_ReadRegister(fHandle, CAEN_DGTZ_ACQ_STATUS_ADD, &acqStatus);
	if(retcode != CAEN_DGTZ_Success)
	{
		TLOG_WARNING("CAENV1730Readout") << "Trying ReadRegister ACQUISITION_STATUS again." << TLOG_ENDL;
		retcode = CAEN_DGTZ_ReadRegister(fHandle, CAEN_DGTZ_ACQ_STATUS_ADD, &acqStatus);
	}
	artdaqcaen::CAENDecoder::checkError(retcode, "ReadRegister ACQ_STATUS", fBoardID);

	TLOG_ARB(TSTATUS, TRACE_NAME) << " Acq status = " << acqStatus << TLOG_ENDL;
	return (acqStatus & ACQ_STATUS_MASK_t::EVENT_READY);
}

void artdaqcaen::CAENV1730Readout::start()
{
	if(fVerbosity > 0)
	...
	if((CAEN_DGTZ_AcqMode_t)(fCAEN.acqMode) == CAEN_DGTZ_AcqMode_t::CAEN_DGTZ_SW_CONTROLLED)
	{
		...
		artdaqcaen::CAENDecoder::checkError(retcode, "SWStartAcquisition", fBoardID);
	}
...
void artdaqcaen::CAENV1730Readout::stop()
{
	if(fVerbosity > 0)
...
bool artdaqcaen::CAENV1730Readout::checkHWStatus_()
{
	for(size_t ch = 0; ch < CAENConfiguration::MAX_CHANNELS; ++ch)
	{
		std::ostringstream tempStream;
		tempStream << "CAENV1730.Card" << fBoardID << ".Channel" << ch << ".Temp";
		std::ostringstream statStream;
		statStream << "CAENV1730.Card" << fBoardID << ".Channel" << ch << ".Status";
		std::ostringstream memfullStream;
		memfullStream << "CAENV1730.Card" << fBoardID << ".Channel" << ch << ".MemoryFull";
		...
		TLOG_ARB(TTEMP, TRACE_NAME) << statStream.str() << std::hex << ": 0x" << ch_status[ch] << std::dec << TLOG_ENDL;
...
bool artdaqcaen::CAENV1730Readout::GetData()
{
...
bool artdaqcaen::CAENV1730Readout::readWindowDataBlocks()
{
	if(fail_GetNext)
	{
	...
	TLOG(TGETDATA) << "(FragID=" << fFragmentID << ")" << __func__ << ": Begin.";
	...
	TLOG(TGETDATA) << "(FragID=" << fFragmentID << ")" << __func__ << ": No timeout. TimePollBegin=" << fTimePollBegin
	               << " TimePollEnd=" << fTimePollEnd;
	...
	TLOG(TGETDATA) << "(FragID=" << fFragmentID << ")" << __func__ << "Start while loop read. " << read_data_size;
	while(read_data_size != 0)
	{
		TLOG(TGETDATA) << "(FragID=" << fFragmentID << ")" << __func__ << "Last read data size was " << read_data_size;
		...
		// call ReadData
		TLOG(TGETDATA) << "(FragID=" << fFragmentID << ")" << __func__ << ": Calling ReadData(fHandle=" << fHandle
		               << ",bufp=" << (void*)block->begin << ",&block.size=" << (void*)&(block->size) << ")";

		retcode =
		    CAEN_DGTZ_ReadData(fHandle, CAEN_DGTZ_SLAVE_TERMINATED_READOUT_MBLT, (char*)block->begin, &read_data_size);
		...
		TLOG(TGETDATA) << "(FragID=" << fFragmentID << ")" << __func__ << "This read data size was " << read_data_size;
		...
		if(fUseTimeTagForTimeStamp)
		{
			...
			fTS = fMeanPollTime - fMeanPollTimeNS + fTTT_ns +
			      ...
			      (fTTT_ns - (long)fMeanPollTimeNS > 500000000) * 1000000000 - fTimeOffsetNanoSec;
		}
		else
		{
		...
		TLOG(TGETDATA) << "(FragID=" << fFragmentID << ")"
		               ...
		               << fTimeDiffPollEnd.total_nanoseconds() << "/" << fMeanPollTime << "/" << fMeanPollTimeNS;
		TLOG(TGETDATA) << "(FragID=" << fFragmentID << ")"
		               << "TIMESTAMP " << fFragmentID << ": TTT/TTT_ns/TS_ns = " << fTTT << "/" << fTTT_ns << "/"
		               << fTS;
		TLOG(TGETDATA) << "(FragID=" << fFragmentID << ")"
		               << "TIMESTAMP " << fFragmentID << ": Timestamp for event " << header->eventCounter << " = "
		               << fTS;
		...
		{
			TLOG(TLVL_DEBUG) << "(FragID=" << fFragmentID << ")" << __func__
	...
	TLOG(TGETDATA) << "(FragID=" << fFragmentID << ")" << __func__ << ": n_reads=" << n_reads;
...
bool artdaqcaen::CAENV1730Readout::readSingleWindowDataBlock()
{
	if(fail_GetNext)
	{
		TLOG(TLVL_ERROR) << "(FragID=" << fFragmentID << ")" << __func__
	...
	while(--fragment_count)
	{
		...
		TLOG(TGETDATA) << "(FragID=" << fFragmentID << ")" << __func__ << "TIMESTAMP " << fFragmentID
		               << ": Poll begin/end/mean/ns = " << fTimeDiffPollBegin.total_nanoseconds() << "/"
		               << fTimeDiffPollEnd.total_nanoseconds() << "/" << fMeanPollTime << "/" << fMeanPollTimeNS;
		TLOG(TGETDATA) << "(FragID=" << fFragmentID << ")"
		               << "TIMESTAMP " << fFragmentID << ": TTT/TTT_ns/TS_ns = " << fTTT << "/" << fTTT_ns << "/"
...
bool artdaqcaen::CAENV1730Readout::getNext_(artdaq::FragmentPtrs& fragments)
{
	if(fail_GetNext)
...
bool artdaqcaen::CAENV1730Readout::readSingleWindowFragments(artdaq::FragmentPtrs& fragments)
{
	...
	while(fPoolBuffer.activeBlockCount())
	{
		...
		auto fragment_uptr = artdaq::Fragment::FragmentBytes(
		    fragment_datasize_bytes, fEvCounter, fFragmentID, artdaqcaen::detail::FragmentType::CAENV1730, metadata);

		using artdaqcaen::PoolBuffer;
		...
		{
			TLOG(TLVL_WARNING) << " TIMESTAMP FOR SEQID " << readoutwindow_event_counter
			                   << " not found in fTimestampMap!"
		...
		else
		{
			...
			if(fUseTimeTagForTimeStamp)
			{
				...
				TLOG_ARB(TMAKEFRAG, TRACE_NAME)
				    << "time offset = " << t_offset_ticks << " ns since the epoch" << TLOG_ENDL;
		...
		else if((ts_now - ts_frag) > 5e9)
		{
			TLOG(TLVL_ERROR) << "Fragment being packged more than 5 seconds after timestamp: "
			                 << "ts_now - ts_frag = " << ts_now - ts_frag << " ns!" << TLOG_ENDL;
		}
		...
		if(readoutwindow_event_counter_gap > 1u)
		{
			if(last_sent_rwcounter > 0)
			{
				TLOG(TLVL_DEBUG) << __func__
				                 << ": Missing data; previous fragment sequenceID / gap  = " << last_sent_rwcounter
				                 << " / " << readoutwindow_event_counter_gap;
				metricMan->sendMetric("Missing Fragments",
				                      uint64_t{readoutwindow_event_counter_gap},
				                      "frags",
				                      1,
		...
		{
			TLOG(TLVL_ERROR) << __func__ << " SequnceIDs processed out of order!! " << readoutwindow_event_counter
			                 << " < " << last_sent_rwcounter << TLOG_ENDL;
		}
		if(last_sent_ts > ts_frag)
		{
			TLOG(TLVL_ERROR) << __func__ << " Timestamps out of order!! Last event later than current one." << ts_frag
			                 << " < " << last_sent_ts << TLOG_ENDL;
		}
...
bool artdaqcaen::CAENV1730Readout::readCombinedWindowFragments(artdaq::FragmentPtrs&)
{
	throw std::runtime_error("Implement me correctly!");
}

void artdaqcaen::CAENV1730Readout::CheckReadback(
    std::string label, int boardID, uint32_t wrote, uint32_t readback, int channelID)
{
	if(wrote != readback)
	{
		...
		// artdaqcaen::CAENException e(CAEN_DGTZ_DigitizerNotReady,
		//			     text.str(), boardId);
		// throw(e);
	}
}

DEFINE_ARTDAQ_COMMANDABLE_GENERATOR(artdaqcaen::CAENV1730Readout)
