...
class artdaqcaen::CAENV1730WaveformAna : public art::EDAnalyzer
{
  public:
...
artdaqcaen::CAENV1730WaveformAna::CAENV1730WaveformAna(fhicl::ParameterSet const& pset) : EDAnalyzer(pset)
{
	...
	nt_wvfm =
	    tfs->make<TNtuple>("nt_wvfm", "Waveform information Ntuple", "art_ev:caen_ev:caen_ev_tts:ch:ped:rms:temp");
}

artdaqcaen::CAENV1730WaveformAna::~CAENV1730WaveformAna() {}

void artdaqcaen::CAENV1730WaveformAna::analyze(art::Event const& evt)
{
	...
	std::cout << "Run " << evt.run() << ", subrun " << evt.subRun() << ", event " << eventNumber << " has "
	          << rawFragHandle->size() << " fragment(s)." << std::endl;

	for(size_t idx = 0; idx < rawFragHandle->size(); ++idx)
	{  // loop over the fragments of an event
		...
		for(size_t i_ch = 0; i_ch < nChannels; ++i_ch)
		{
			...
			{
				if(i_t % 2 == 0)
					fWvfmsVec[i_ch][i_t] = *(data + n_samples + i_t + 1);
				else if(i_t % 2 == 1)
					fWvfmsVec[i_ch][i_t] = *(data + n_samples + i_t - 1);
			}
			...
			// get mean
			float wvfm_mean =
			    std::accumulate(fWvfmsVec[i_ch].begin(), fWvfmsVec[i_ch].end(), 0.0) / fWvfmsVec[i_ch].size();
			...
			nt_wvfm->Fill(
			    eventNumber, header.eventCounter, header.triggerTimeTag, i_ch, wvfm_mean, wvfm_rms, md->chTemps[i_ch]);
		}
	}
}

DEFINE_ART_MODULE(artdaqcaen::CAENV1730WaveformAna)
