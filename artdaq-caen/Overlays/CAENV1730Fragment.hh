#ifndef artdaq_caen_Overlays_CAENV1730Fragment_hh
#define artdaq_caen_Overlays_CAENV1730Fragment_hh
...
struct artdaqcaen::CAENV1730EventHeader
{
...
static_assert(sizeof(artdaqcaen::CAENV1730EventHeader) == 4 * sizeof(uint32_t),
              "CAENV1730EventHeader not correct size.");

struct artdaqcaen::CAENV1730Event
{
	CAENV1730EventHeader Header;
	uint16_t             DataBlock;
};

struct artdaqcaen::CAENV1730FragmentMetadata
{
	uint32_t nChannels;
	uint32_t nSamples;
	...
	CAENV1730FragmentMetadata(
	    uint32_t nc, uint32_t ns, uint32_t ts_s, uint32_t ts_ns, uint32_t chtemp[CAEN_V1730_MAX_CHANNELS])
	...
	size_t ExpectedDataSize() const { return (sizeof(CAENV1730EventHeader) + nChannels * nSamples * sizeof(uint16_t)); }
};

class artdaqcaen::CAENV1730Fragment
{
  public:
	...
	{
		return fFragment.metadata<artdaqcaen::CAENV1730FragmentMetadata>();
	}

	CAENV1730Event const* Event() const { return reinterpret_cast<CAENV1730Event const*>(fFragment.dataBeginBytes()); }
