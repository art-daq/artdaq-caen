...
class artdaqcaen::CAENV1730Dump : public art::EDAnalyzer
{
  public:
	struct Config
	{
...
artdaqcaen::CAENV1730Dump::CAENV1730Dump(CAENV1730Dump::Parameters const& pset) : art::EDAnalyzer(pset)
{
	fDataLabel = pset().DataLabel();
	fShift     = pset().Shift();
}

void artdaqcaen::CAENV1730Dump::beginJob()
{
...
void artdaqcaen::CAENV1730Dump::endJob() { std::cout << "Ending CAENV1730Dump...\n"; }

artdaqcaen::CAENV1730Dump::~CAENV1730Dump() {}

void artdaqcaen::CAENV1730Dump::analyze(const art::Event& evt)
{
	...
	{
		std::cout << "######################################################################\n";
		std::cout << "Run " << evt.run() << ", subrun " << evt.subRun() << ", event " << fEvent << " has "
		          << rawFragHandle->size() << " fragment(s).\n";

		// std::fstream
		// dumpouttext("/home/nfs/aw325/aiwudaq_vst02/srcs/sbndaq/dab/Analysis/DumpIntoText.txt",std::ios::out |
		// std::ios::app);
		...
		{ /*loop over the fragments*/
			...
			const uint16_t* data_begin =
			    reinterpret_cast<const uint16_t*>(frag.dataBeginBytes() + sizeof(CAENV1730EventHeader));
			...
			for(size_t i_ch = 0; i_ch < nChannels; ++i_ch)
			{
				...
				for(size_t i_t = 0; i_t < wfm_length; ++i_t)
				{
					...
					if(i_ch == 0 && firstEvt)
					{
						...
						// std::cout << "ch" << std::to_string(i_ch) << "[" << i_t << "] = " << value <<  "= 0b" <<
						// std::bitset<16>(value)
						//           << std::endl;
					}
...
DEFINE_ART_MODULE(artdaqcaen::CAENV1730Dump)
