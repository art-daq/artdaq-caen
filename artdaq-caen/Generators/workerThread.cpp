...
void share::WorkerThread::stop()
{
	...
	int availableRetryCount =
	    _maxStopAttempts;  // if the thread function is still runing give it another 10 seconds to complete
...
void share::WorkerThread::run()
{
	...
	{
		std::cout << "WorkerThread: "
		          << "Caught exception while running " << _functor->name() << ". Nested exception:Unknown" << std::endl;
	}
