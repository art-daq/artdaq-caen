...
#include "artdaq-caen/Overlays/CAENV1720Fragment.hh"
...
class artdaqcaen::CAENV1720Dump : public art::EDAnalyzer
{
  public:
	struct Config
	{
		//--one atom for each parameter
		fhicl::Atom<art::InputTag> DataLabel{fhicl::Name("data_label"),
		                                     fhicl::Comment("Tag for the input data product")};
		fhicl::Atom<int>           Shift{fhicl::Name("shift_fragment_id"),
                               fhicl::Comment("Number to subtract to the fragment_id")};
		fhicl::Atom<int>           EventToPlot{fhicl::Name("event_to_plot"),
                                     fhicl::Comment("Event number to be shown in histograms")};
		fhicl::Atom<int> NumberOfBoards{fhicl::Name("number_of_boards"), fhicl::Comment("Number of V1720 boards ")};
		fhicl::Atom<int> NumberOfChannels{fhicl::Name("number_of_channels"),
		                                  fhicl::Comment("Number of V1720 channels")};
		fhicl::Atom<int> WaveformLength{fhicl::Name("waveform_length"),
...
artdaqcaen::CAENV1720Dump::CAENV1720Dump(CAENV1720Dump::Parameters const& pset) : art::EDAnalyzer(pset)
{
	...
	{
		TLOG(TLVL_ERROR) << "MaxnChannels = " << (int)MaxnChannels << " exceeds maximum allowed size of "
		                 << MAX_MAX_N_CHANNELS;
	}
...
void artdaqcaen::CAENV1720Dump::beginJob()
{
	...
	for(int board = 0; board < number_of_boards; board++)
		for(int ch = 0; ch < MaxnChannels; ch++)
			h_wvfm_ev0[ch + board * MaxnChannels] =
			    tfs->make<TH1S>(TString::Format("h_wvfm_ev%d_board%d_ch%d", EventToPlot, board, ch),
			                    TString::Format("Waveform for event %d, board %d, channel %d", EventToPlot, board, ch),
			                    waveform_length,
			                    0,
			                    waveform_length);
...
void artdaqcaen::CAENV1720Dump::endJob() { std::cout << "Ending CAENV1720Dump...\n"; }

artdaqcaen::CAENV1720Dump::~CAENV1720Dump() {}

void artdaqcaen::CAENV1720Dump::analyze(const art::Event& evt)
{
	fRun    = evt.run();
	fSubrun = evt.subRun();
	fEvent  = evt.event();
	...
	for(const auto& handle : fragmentHandles)
	{
		...
		else if(handle->front().type() == artdaqcaen::detail::FragmentType::CAENV1720)
		{
			...
			{  // loop over fragments
				...
				TLOG(TLVL_INFO) << "\n\tFrom fragment header, typeString " << frag.typeString()
				                << "\n\tFrom fragment header, timestamp  " << (frag.timestamp() / 1'000'000'000)
				                << " s, " << std::setfill('0') << std::setw(9) << (frag.timestamp() % 1'000'000'000)
				                << std::setw(0) << " ns"
				                ...
				                << "\n\tFrom metadata, chTemps           " << md->chTemps[0] << ", " << md->chTemps[1]
				                << ", " << md->chTemps[2] << ", " << md->chTemps[3] << ", " << md->chTemps[4] << ", "
				                << md->chTemps[5] << ", " << md->chTemps[6] << ", " << md->chTemps[7];
				fFragmentID -= fShift;
				...
				const uint16_t* data_begin =
				    reinterpret_cast<const uint16_t*>(frag.dataBeginBytes() + sizeof(CAENV1720EventHeader));
				...
				for(size_t i_ch = 0; i_ch < nChannels; ++i_ch)
				{
					...
					{
						TLOG(TLVL_ERROR) << "found channel " << i_ch << " larger than " << (MaxnChannels)
						                 << "! How could it happen? Debug, debug!";
						break;
					}
...
DEFINE_ART_MODULE(artdaqcaen::CAENV1720Dump)
