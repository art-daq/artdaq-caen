#ifndef artdaq_caen_Overlays_CAENV1720Fragment_hh
#define artdaq_caen_Overlays_CAENV1720Fragment_hh
...
struct artdaqcaen::CAENV1720EventHeader
{
	...
	uint64_t extendedTriggerTime() const { return triggerTimeTag + (static_cast<uint64_t>(pattern) << 32U); }
};
static_assert(sizeof(artdaqcaen::CAENV1720EventHeader) == 4 * sizeof(uint32_t),
              "CAENV1720EventHeader not correct size.");

struct artdaqcaen::CAENV1720Event
{
	CAENV1720EventHeader Header;
	uint16_t             DataBlock;
};

struct artdaqcaen::CAENV1720FragmentMetadata
{
	uint32_t boardID;
	uint32_t nChannels;
	uint32_t nSamples;
	...
	CAENV1720FragmentMetadata(
	    uint32_t bid, uint32_t nc, uint32_t ns, uint32_t ts_s, uint32_t ts_ns, uint32_t chtemp[CAEN_V1720_MAX_CHANNELS])
	    : boardID(bid), nChannels(nc), nSamples(ns), timeStampSec(ts_s), timeStampNSec(ts_ns)
	{
		memcpy(chTemps, chtemp, CAEN_V1720_MAX_CHANNELS * sizeof(uint32_t));
	}

	size_t ExpectedDataSize() const { return (sizeof(CAENV1720EventHeader) + nChannels * nSamples * sizeof(uint16_t)); }
};

class artdaqcaen::CAENV1720Fragment
{
  public:
	...
	{
		return fFragment.metadata<artdaqcaen::CAENV1720FragmentMetadata>();
	}

	CAENV1720Event const* Event() const { return reinterpret_cast<CAENV1720Event const*>(fFragment.dataBeginBytes()); }

	size_t DataPayloadSize() const { return fFragment.dataSizeBytes(); }

	size_t ExpectedDataSize() const { return Metadata()->ExpectedDataSize(); }
