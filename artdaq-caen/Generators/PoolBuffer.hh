  ...
  private:
	void deallocate()
	{
		...
		while(remainingBlockCount)
		{
			...
			while(!_freeDataBlocks.empty())
			{
				_freeDataBlocks.pop();
				remainingBlockCount--;
			}
			while(!_activeDataBlocks.empty())
			{
				_activeDataBlocks.pop();
				remainingBlockCount--;
			}
			std::this_thread::yield();
			TLOG(TLVL_DEBUG) << __func__ << ": Waiting for " << remainingBlockCount
			                 << " DataBlocks to be rerurned to PoolBuffer";
		}
...
std::size_t PoolBuffer::read(DataRange<T>& range)
{
	...
	{
		TLOG(TLVL_ERROR) << __func__ << ": Not enough space; DataRange.size=" << range_size
		                 << " is smaller than DataBlock.size=" << _blockSize;
...
inline std::size_t PoolBuffer::allocate(std::size_t block_size,
                                        std::size_t poolbuffer_size = max_buffer_size,
                                        bool        enable_redzones = false)
{
	if(_poolBufferSize != 0)
		throw std::logic_error("Reallocation of PoolBuffer is not allowed.");
	...
	if(!_rawPoolBuffer)
		throw std::length_error("Failed allocating "s + std::to_string(poolbuffer_size) + " bytes."s);
	_poolBufferSize = poolbuffer_size;

	auto buf = _rawPoolBuffer.get();

	for(std::size_t i = 0; i < _poolBufferSize; i += 64)
		buf[i] = '*';
	...
	for(std::size_t i = 0; i < _blockCount; i++)
	{
		...
		if(_enableRedzones != 0)
		{
			std::copy(redzone_bytes.begin(),
			          redzone_bytes.end(),
			          _allDataBlocks.front()->begin + (block_size - redzone_bytes.size()));
		}
	}
	for(auto& block : _allDataBlocks)
		_freeDataBlocks.push(block);
